...
const Chat: React.FC = () => {
  ...
  const addToShortlist = (name: string) => {
      setShortlist((prevShortlist) => {
          if (!prevShortlist.includes(name)) {
              return [...prevShortlist, name];
          }
          return prevShortlist; // Return the existing shortlist if name already exists
      });
  };
  ...
  return (
    <>
      <Helmet>
        <title>Chatbot - BrandsDigger</title>
        <meta name="description" content="Interact with the BrandsDigger chatbot to discover the perfect business names and ideas. Get personalized suggestions and add them to your shortlist." />
      </Helmet>
              ...
              {messages.map((msg, index) => (
                <div key={index} className={`${msg.sender === 'user' ? styles.userMessage : styles.botMessage} ${styles.responseBox}`}>
                  {msg.sender === 'user' ? (
                    ...
                    msg.text.split('\n').map((line, lineIndex) => {
                      const endsWithValidDomain = /\.[a-zA-Z]{2,}$/.test(line.trim()); // Regex for matching any domain extension
